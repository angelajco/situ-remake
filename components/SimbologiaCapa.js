...
class RangoSimb {
	...
	constructor(tipo, Min, Max, color, leyenda, borde, grosor, radio, tipoLin) {
		if (borde === undefined) {
...
class simbologiaCapa {
	...
	agregaRango(tipo, valorMin, valorMax, color, leyenda, borde, grosor, radio, tipoLin) {
		let nuevo = new RangoSimb(tipo, valorMin, valorMax, color, leyenda, borde, grosor, radio, tipoLin);
		this.rangos.push(nuevo);
	}
	...
	tablahtml(aux) {
		...
		if (aux == 0) {
			...
			let aux1 = 0;
			let salida = `<table id='tablaE' class="table-wrapper-scroll-y my-custom-scrollbar"><tr><th>Minimo</th><th>Maximo</th><th>Leyenda</th><th>Color</th><th>Borde</th><th>Grosor</th><th>Radio</th><th></th><tr>`;
			for (let i = 0; i < this.rangos.length; i++) {
	...
	generaCuantiles(numDivisiones, arreglo, colores, leyenda, borde, grosor, radio, tipoLin) {
		...
		for (let i = 1; i < arreglo.length; i++) {
			if (arreglo[i] != null) {
				if (cont2 < longRango || arreglo[i] == valorMin) {
					...
					simbologia.agregaRango(0, valorMin, valorMaximo, colores[cont1], (leyenda + div), borde, grosor, radio, tipoLin);
					cont2 = 1;
					valorMin = arreglo[i];
